...
/**
 * Item Model for an Employee_absence.
 *
 * @since  1.6
 */
class HrModelEmployee_absence extends JModelAdmin
{
	/**
	...
	/**
	 * The type alias for this content type (for example, 'com_hr.employee_absence').
	 *
	 * @var      string
	 * @since    3.2
	 */
	public $typeAlias = 'com_hr.employee_absence';
...
public function delete(&$pks)
	...
	$pks = (array) $pks;
		...
		try
		{
			...
			$db->setQuery(
				'DELETE FROM #__hr_absence_charge' .
					' WHERE id_of_the_absence IN (' . implode(',', $pks) . ')'
			);
			$db->execute();
...
parent::delete($pks);
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// Check if the employee_absence was featured and update the #__hr_employee_absence_frontpage table
			if ($featured == 1)
			{
				$db = $this->getDbo();
				$query = $db->getQuery(true)
					->insert($db->quoteName('#__hr_employee_absence_frontpage'))
					->values($newId . ', 0');
				$db->setQuery($query);
				$db->execute();
			}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_hr.employee_absence.' . (int) $record->id);
		}
	...
	protected function canEditState($record)
	{
		...
		// Check for existing employee_absence.
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_hr.employee_absence.' . (int) $record->id);
		}
		// New employee_absence, so check against the category.
		elseif (!empty($record->catid))
		{
			return $user->authorise('core.edit.state', 'com_hr.category.' . (int) $record->catid);
		}
		// Default to component settings if neither employee_absence nor category known.
		else
		{
...
//jexit();               
        ...
        $query->where('(a.id != ' . $id . ') ');
        $query->where('(final_approval_status != ' . '"Rejected"' . ') ');
...
//jexit();           
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$item->employee_absencetext = trim($item->fulltext) != '' ? $item->introtext . "<hr id=\"system-readmore\" />" . $item->fulltext : $item->introtext;

                      
			if (!empty($item->id))
			{
				$item->tags = new JHelperTags;
				$item->tags->getTagIds($item->id, 'com_hr.employee_absence');
			}
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_hr', '#__hr_employee_absence', 'com_hr.item', $item->id);
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hr.employee_absence', 'employee_absence', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($this->getState('employee_absence.id'))
		{
			$id = $this->getState('employee_absence.id');

			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');

			// Existing record. Can only edit own employee_absences in selected categories.
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_hr.employee_absence.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_hr')))
		{
			...
			// The controller has already verified this is an employee_absence you can edit.
		...
		// Prevent messing with employee_absence language and category when editing existing employee_absence with associations
		$app = JFactory::getApplication();
		$assoc = JLanguageAssociations::isEnabled();

		// Check if employee_absence is associated
		if ($this->getState('employee_absence.id') && $app->isSite() && $assoc)
		{
			$associations = JLanguageAssociations::getAssociations('com_hr', '#__hr_employee_absence', 'com_hr.item', $id);
	...
	protected function loadFormData()
	{
		...
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_hr.edit.employee_absence.data', array());
		...
		if (empty($data))
		{
          ...
          //      jexit();
           ...
           //            ////var_depr_dump($data->datetime_to);
			...
			// Pre-select some filters (Status, Category, Language, Access) in edit form if those have been selected in Employee_absence Manager: Employee_absences
			if ($this->getState('employee_absence.id') == 0)
			{
				$filters = (array) $app->getUserState('com_hr.employee_absences.filter');
				$data->set(
					'state',
					$app->input->getInt(
						'state',
     ...
     if (is_object($data)) {              
          if ($data->id == NULL) {
            ...
            $query->where($db->quoteName('title') . ' = ' . "'Absences of Employees'");
            $db->setQuery($query);
            $results = $db->loadObjectList();
            foreach ($results as $result) {
		...
		$this->preprocessData('com_hr.employee_absence', $data);
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (!isset($data['id']) || (int) $data['id'] == 0))
		{
			if ($data['alias'] == null)
			{
				...
				$table = JTable::getInstance('Employee_absence', 'HrTable');
	...
	/**
	 * Method to toggle the featured setting of employee_absences.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		$table = $this->getTable('Employee_absence', 'HrTable');

		try
		{
			$db = $this->getDbo();
			$query = $db->getQuery(true)
				->update($db->quoteName('#__hr_employee_absence'))
			...
			if ((int) $value == 0)
			{
				...
				$query = $db->getQuery(true)
					->delete($db->quoteName('#__hr_employee_absence_frontpage'))
			...
			{
				// First, we find out which of our new featured employee_absences are already featured.
				$query = $db->getQuery(true)
					->select('f.ms_id')
					->from('#__hr_employee_absence_frontpage AS f')
					->where('ms_id IN (' . implode(',', $pks) . ')');
				$db->setQuery($query);

				$old_featured = $db->loadColumn();

				// We diff the arrays to get a list of the employee_absences that are newly featured
		...
		if ($assoc)
		{
			...
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) || $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_employee_absence');
	...
	//	parent::cleanCache('mod_employee_absences_archive');
	//	parent::cleanCache('mod_employee_absences_categories');
	//	parent::cleanCache('mod_employee_absences_category');
	//	parent::cleanCache('mod_employee_absences_latest');
	//	parent::cleanCache('mod_employee_absences_news');
	//	parent::cleanCache('mod_employee_absences_popular');
	}
