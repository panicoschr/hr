...
/**
 * Item Model for an Employee_roster.
 *
 * @since  1.6
 */
class HrModelEmployee_roster extends JModelAdmin
{
	/**
	...
	/**
	 * The type alias for this content type (for example, 'com_hr.employee_roster').
	 *
	 * @var      string
	 * @since    3.2
	 */
	public $typeAlias = 'com_hr.employee_roster';
   ...
   if ($date1 >= $date2) {
	...
	require_once JPATH_COMPONENT.'/helpers/hr.php';
        ...
        $query->select($db->quoteName(array('a.employee_id', 'u.name', 'a.datetime_from', 'a.datetime_to')));
...
//$query->where('(a.employee_id = '."'$employee_id'".' OR a.position_id = '."'$position_id'".')');
 ...
 //      $query->select('p.title AS position_title');     
      ...
      //      $position_id = $result->position_id;
            ...
            $datetimerosterfrom = $result->datetime_from;
            $datetimerosterto = $result->datetime_to;
 ...
 $datetime_to  = new DateTime($datetime_to_string);  
            ...
            $count = $count + 1;
                    $keepDatesTrack = $keepDatesTrack . " " . $uname. " holds position " . $datetimerosterfrom. " from " . $formatedfromdate . " to " . $formatedtodate .  ", " ;
                }
            }
            if ($datetime_from == $datetime_to) {
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// Check if the employee_roster was featured and update the #__hr_employee_roster_frontpage table
			if ($featured == 1)
			{
				$db = $this->getDbo();
				$query = $db->getQuery(true)
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_hr.employee_roster.' . (int) $record->id);
		}
	...
	protected function canEditState($record)
	{
		...
		// Check for existing employee_roster.
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_hr.employee_roster.' . (int) $record->id);
		}
		// New employee_roster, so check against the category.
		elseif (!empty($record->catid))
		{
			return $user->authorise('core.edit.state', 'com_hr.category.' . (int) $record->catid);
		}
		// Default to component settings if neither employee_roster nor category known.
		else
		{
	...
	protected function prepareTable($table)
	{
		...
		$user	= JFactory::getUser();
            ...
            $table->datetime_from = $datetime_from_new;  
		...
		// Reorder the employee_rosters within the category so the new employee_roster is first
		if (empty($table->id))
		{
	 ...
	 */
	public function getTable($type = 'Employee_roster', $prefix = 'HrTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$item->employee_rostertext = trim($item->fulltext) != '' ? $item->introtext . "<hr id=\"system-readmore\" />" . $item->fulltext : $item->introtext;

                      
			if (!empty($item->id))
			{
				$item->tags = new JHelperTags;
				$item->tags->getTagIds($item->id, 'com_hr.employee_roster');
			}
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_hr', '#__hr_employee_roster', 'com_hr.item', $item->id);
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hr.employee_roster', 'employee_roster', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($this->getState('employee_roster.id'))
		{
			$id = $this->getState('employee_roster.id');

			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');

			// Existing record. Can only edit own employee_rosters in selected categories.
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_hr.employee_roster.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_hr')))
		{
			...
			// The controller has already verified this is an employee_roster you can edit.
		...
		// Prevent messing with employee_roster language and category when editing existing employee_roster with associations
		$app = JFactory::getApplication();
		$assoc = JLanguageAssociations::isEnabled();

		// Check if employee_roster is associated
		if ($this->getState('employee_roster.id') && $app->isSite() && $assoc)
		{
			$associations = JLanguageAssociations::getAssociations('com_hr', '#__hr_employee_roster', 'com_hr.item', $id);
	...
	protected function loadFormData()
	{
		...
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_hr.edit.employee_roster.data', array());
		...
		if (empty($data))
		{
          ...
          //      jexit();
           ...
           //            ////var_depr_dump($data->datetime_to);
			...
			// Pre-select some filters (Status, Category, Language, Access) in edit form if those have been selected in Employee_roster Manager: Employee_rosters
			if ($this->getState('employee_roster.id') == 0)
			{
				$filters = (array) $app->getUserState('com_hr.employee_rosters.filter');
				$data->set(
					'state',
					$app->input->getInt(
						'state',
		...
		$this->preprocessData('com_hr.employee_roster', $data);
	...
	/**
	 * Method to toggle the featured setting of employee_rosters.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		try
		{
			...
			{
				// First, we find out which of our new featured employee_rosters are already featured.
				$query = $db->getQuery(true)
					->select('f.ms_id')
				...
				// We diff the arrays to get a list of the employee_rosters that are newly featured
		...
		if ($assoc)
		{
			...
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) || $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_employee_roster');
	...
	//	parent::cleanCache('mod_employee_rosters_archive');
	//	parent::cleanCache('mod_employee_rosters_categories');
	//	parent::cleanCache('mod_employee_rosters_category');
	//	parent::cleanCache('mod_employee_rosters_latest');
	//	parent::cleanCache('mod_employee_rosters_news');
	//	parent::cleanCache('mod_employee_rosters_popular');
	}
