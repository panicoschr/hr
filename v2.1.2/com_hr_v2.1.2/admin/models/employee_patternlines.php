...
/**
 * Methods supporting a list of employee_patternline records.
 *
 * @since  1.6
 */
class HrModelEmployee_patternlines extends JModelList
{
	/**
	 * Constructor.
	 *
...
$stringofarrayofcatandsubcats = implode("' , '", $arrayofcatandsubcats);
		...
		$query->from('#__hr_employee_patternline AS a');
		...
		$query->select('ua.name AS author_name')
			->join('LEFT', '#__users AS ua ON ua.id = a.created_by');
                ...
                $query->select('a.title AS employee_patternline_title');
...
// Filter by employee
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_employee_patternlineitem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.employee_patternline')
				);
		}
*/
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__hr_employee_patternline AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	/**
	 * Method to get a list of employee_patternlines.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove employee_patternlines the user shouldn't see
