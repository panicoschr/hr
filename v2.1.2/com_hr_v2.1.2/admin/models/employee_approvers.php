...
/**
 * Methods supporting a list of employee_approver records.
 *
 * @since  1.6
 */
class HrModelEmployee_approvers extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'approver1', 'a.approver1_id',                               
				'approver2', 'a.approver2_id',                               
				'approver3', 'a.approver3_id',                               
				'approver4', 'a.approver4_id',                               
				'approver5', 'a.approver5_id', 
		...
		$query->select(
			$this->getState(
				...
				'a.id, a.title, a.alias, a.checked_out, a.checked_out_time, a.catid' .
					...
					', a.publish_up, a.publish_down, d.abstype' .
             ...
             where usrs.id = a.employee_id)' .        
               ...
               ', (SELECT app1s.name as app1name
                       FROM  #__users As app1s
             where app1s.id = a.approver1_id)' .                   
                                
               ', (SELECT app2s.name as app2name
                       FROM  #__users As app2s
             where app2s.id = a.approver2_id)' .        
                                
               ', (SELECT app3s.name as app3name
                       FROM  #__users As app3s
             where app3s.id = a.approver3_id)' .                                  
                                 
               ', (SELECT app4s.name as app4name
                       FROM  #__users As app4s
             where app4s.id = a.approver4_id)' .                                 
                                
              ', (SELECT app5s.name as app5name
                       FROM  #__users As app5s
             where app5s.id = a.approver5_id)'                                 
                                        
			)
		);
		$query->from('#__hr_employee_approver AS a');
...
$query->join('INNER', $db->quoteName('#__hr_absence', 'd') . ' ON (' . $db->quoteName('a.absence_id') . ' = ' . $db->quoteName('d.id') . ')');                
                ...
                $query->select('a.title AS employee_approver_title');
...
$query->select('(SELECT app1s.name as app1name
                       FROM  #__users As app1s
             where app1s.id = a.approver1_id)  AS approver1');  

$query->select('(SELECT app2s.name as app2name
                       FROM  #__users As app2s
             where app2s.id = a.approver2_id)  AS approver2'); 

$query->select('(SELECT app3s.name as app3name
                       FROM  #__users As app3s
             where app3s.id = a.approver3_id)  AS approver3');  

$query->select('(SELECT app4s.name as app4name
                       FROM  #__users As app4s
             where app4s.id = a.approver4_id)  AS approver4');  

$query->select('(SELECT app5s.name as app5name
                       FROM  #__users As app5s
             where app5s.id = a.approver5_id)  AS approver5');  
       ...
       $abstype = $this->getState('filter.abstype');
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_employee_approveritem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.employee_approver')
				);
		}
*/
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__hr_employee_approver AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	/**
	 * Method to get a list of employee_approvers.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove employee_approvers the user shouldn't see
