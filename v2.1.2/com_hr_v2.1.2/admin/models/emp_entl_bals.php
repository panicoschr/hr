...
/**
 * Methods supporting a list of emp_entl_bal records.
 *
 * @since  1.6
 */
...
$query->select('a.ref_year AS ref_year');    
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_emp_entl_balitem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.emp_entl_bal')
				);
		}
*/
...
where FOO2.oneid = CONCAT(a.employee_id, a.absence_id, a.ref_year)                       
	...
	/**
	 * Method to get a list of emp_entl_bals.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove emp_entl_bals the user shouldn't see
