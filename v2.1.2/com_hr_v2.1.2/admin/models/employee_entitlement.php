...
/**
 * Item Model for an Employee_entitlement.
 *
 * @since  1.6
 */
class HrModelEmployee_entitlement extends JModelAdmin
{
	/**
	...
	/**
	 * The type alias for this content type (for example, 'com_hr.employee_entitlement').
	 *
	 * @var      string
	 * @since    3.2
	 */
	public $typeAlias = 'com_hr.employee_entitlement';
...
//F
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// Check if the employee_entitlement was featured and update the #__hr_employee_entitlement_frontpage table
			if ($featured == 1)
			{
				$db = $this->getDbo();
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_hr.employee_entitlement.' . (int) $record->id);
		}
	...
	protected function canEditState($record)
	{
		...
		// Check for existing employee_entitlement.
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_hr.employee_entitlement.' . (int) $record->id);
		}
		// New employee_entitlement, so check against the category.
		elseif (!empty($record->catid))
		{
			return $user->authorise('core.edit.state', 'com_hr.category.' . (int) $record->catid);
		}
		// Default to component settings if neither employee_entitlement nor category known.
		else
		{
	...
	protected function prepareTable($table)
	{
       ...
       $table->title =  $nowstring;                
		...
		// Reorder the employee_entitlements within the category so the new employee_entitlement is first
		if (empty($table->id))
		{
	 ...
	 */
	public function getTable($type = 'Employee_entitlement', $prefix = 'HrTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$item->employee_entitlementtext = trim($item->fulltext) != '' ? $item->introtext . "<hr id=\"system-readmore\" />" . $item->fulltext : $item->introtext;

                      
			if (!empty($item->id))
			{
				$item->tags = new JHelperTags;
				$item->tags->getTagIds($item->id, 'com_hr.employee_entitlement');
			}
		}
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hr.employee_entitlement', 'employee_entitlement', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($this->getState('employee_entitlement.id'))
		{
			$id = $this->getState('employee_entitlement.id');

			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');

			// Existing record. Can only edit own employee_entitlements in selected categories.
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_hr.employee_entitlement.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_hr')))
		{
			...
			// The controller has already verified this is an employee_entitlement you can edit.
		...
		// Prevent messing with employee_entitlement language and category when editing existing employee_entitlement with associations
		$app = JFactory::getApplication();
		$assoc = JLanguageAssociations::isEnabled();

		// Check if employee_entitlement is associated
		if ($this->getState('employee_entitlement.id') && $app->isSite() && $assoc)
		{
	...
	protected function loadFormData()
	{
		...
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_hr.edit.employee_entitlement.data', array());

		if (empty($data))
		{
			$data = $this->getItem();

			// Pre-select some filters (Status, Category, Language, Access) in edit form if those have been selected in Employee_entitlement Manager: Employee_entitlements
			if ($this->getState('employee_entitlement.id') == 0)
			{
				$filters = (array) $app->getUserState('com_hr.employee_entitlements.filter');
				$data->set(
					'state',
					$app->input->getInt(
						'state',
   ...
   if (is_object($data)) {
		...
		$this->preprocessData('com_hr.employee_entitlement', $data);
	...
	/**
	 * Method to toggle the featured setting of employee_entitlements.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		try
		{
			...
			{
				// First, we find out which of our new featured employee_entitlements are already featured.
				$query = $db->getQuery(true)
					->select('f.ms_id')
				...
				// We diff the arrays to get a list of the employee_entitlements that are newly featured
		...
		if ($assoc)
		{
			...
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) || $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_employee_entitlement');
	...
	//	parent::cleanCache('mod_employee_entitlements_archive');
	//	parent::cleanCache('mod_employee_entitlements_categories');
	//	parent::cleanCache('mod_employee_entitlements_category');
	//	parent::cleanCache('mod_employee_entitlements_latest');
	//	parent::cleanCache('mod_employee_entitlements_news');
	//	parent::cleanCache('mod_employee_entitlements_popular');
	}
