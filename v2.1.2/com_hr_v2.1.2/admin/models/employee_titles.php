...
/**
 * Methods supporting a list of employee_title records.
 *
 * @since  1.6
 */
class HrModelEmployee_titles extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'category_id',
				'level',
				'tag', 'username', 'a.employee_id',  'a.title_id',  
                            	'datetime_from', 'a.datetime_from',                            
				'datetime_to', 'a.datetime_to',                               
				'title_description', 't.title'      
...
$stringofarrayofcatandsubcats = implode("' , '", $arrayofcatandsubcats);     
		...
		$query->from('#__hr_employee_title AS a');
...
$query->join('LEFT', $db->quoteName('#__hr_title', 't') . ' ON (' . $db->quoteName('a.title_id') . ' = ' . $db->quoteName('t.id') . ')');
  
                $query->select('a.title AS employee_title_title');
                          $query->select('t.title AS ttitle');
...
if ($itemselected == ''){           
 ...
 // Filter by title
        ...
        if ($title != NULL) {
            ...
            $query->where($db->quoteName('t.title') . ' =  ' . $db->quote($title));
        }                
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_employee_titleitem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.employee_title')
				);
		}
*/
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__hr_employee_title AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	/**
	 * Method to get a list of employee_titles.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove employee_titles the user shouldn't see
