...
/**
 * Methods supporting a list of employee_position records.
 *
 * @since  1.6
 */
class HrModelEmployee_positions extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = 'a.id', $direction = 'desc')
	{
		...
		$this->setState('filter.language', $language);
                ...
                $position = $this->getUserStateFromRequest($this->context . '.filter.position', 'filter_position', '');
...
$stringofarrayofcatandsubcats = implode("' , '", $arrayofcatandsubcats);     
		...
		$query->from('#__hr_employee_position AS a');
...
$query->join('LEFT', $db->quoteName('#__hr_position', 'p') . ' ON (' . $db->quoteName('a.position_id') . ' = ' . $db->quoteName('p.id') . ')');                
                ...
                $query->select('a.title AS employee_position_title');
...
$query->select('(SELECT usrs.name as usrname
 ...
 // Filter by position
        ...
        if ($position != NULL) {
            ...
            $query->where($db->quoteName('p.title') . ' =  ' . $db->quote($position));
        }                
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_employee_positionitem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.employee_position')
				);
		}
*/
...
$query->where($db->quoteName('a.employee_id') . ' IN ' . 
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__hr_employee_position AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	/**
	 * Method to get a list of employee_positions.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove employee_positions the user shouldn't see
