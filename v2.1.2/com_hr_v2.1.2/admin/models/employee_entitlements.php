...
/**
 * Methods supporting a list of employee_entitlement records.
 *
 * @since  1.6
 */
class HrModelEmployee_entitlements extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'validity_datetime_from', 'a.validity_datetime_from', 
                                'validity_datetime_to', 'a.validity_datetime_to'  
			...
			$this->getState(
             ...
             where usrs.id = a.employee_id), ' .                                   
                               ...
                               'a.employee_id, ' .
                                ...
                                'a.datetime_from, ' .                                
                                'a.datetime_to, ' .                                
                                'a.validity_datetime_from, ' .                                
                                'a.validity_datetime_to' 
...
//$query->join('LEFT', $db->quoteName('#__users', 'u') . ' ON (' . $db->quoteName('a.employee_id') . ' = ' . $db->quoteName('u.id') . ')');
 ...
 $itemselected = 'supervisor';                 
		...
		$query->select('ua.name AS author_name')
			->join('LEFT', '#__users AS ua ON ua.id = a.created_by');
                ...
                $query->select('a.title AS employee_entitlement_title');
       ...
       $abstype = $this->getState('filter.abstype');
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_employee_entitlementitem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.employee_entitlement')
				);
		}
*/
                
      
   $query->where("CONCAT(a.datetime_from, a.datetime_to)".' != '. "'0000-00-00 00:00:000000-00-00 00:00:00'");                  
                
                
         

$query->where('a.absence_id != '. '0'); 
...
$query->where('a.employee_id != '. '0'); 
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__hr_employee_entitlement AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	/**
	 * Method to get a list of employee_entitlements.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove employee_entitlements the user shouldn't see
