...
/**
 * Methods supporting a list of employee_roster records.
 *
 * @since  1.6
 */
		...
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'tag', 'a.employee_id',
                                  ...
                                  'employee_id', 'a.employee_id', 'roster_datetime_from',
                                  'roster_datetime_to', 'username', 
                                  'absence_datetime_from', 'absence_datetime_to',
                                  'abstype', 'ab.abstype',
                                  'final_approval_status', 'ea.final_approval_status',
                                  'measureunit', 'ab.measureunit', 'datetime_from', 'a.datetime_from',
...
$stringofarrayofcatandsubcats = implode("' , '", $arrayofcatandsubcats);
     ...
     $nowstring=$now->format('Y-m-d');   
		...
		$query->from('#__hr_employee_roster AS a');
...
$query->join('LEFT', $db->quoteName('#__hr_absence', 'ab') . ' ON (' . $db->quoteName('ea.absence_id') . ' = ' . $db->quoteName('ab.id') . ')');
                ...
                $query->select('a.title AS employee_roster_title');
       ...
       $itemselected = 'supervisor';                 
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_employee_rosteritem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.employee_roster')
				);
		}
*/
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__hr_employee_roster AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	/**
	 * Method to get a list of employee_rosters.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove employee_rosters the user shouldn't see
