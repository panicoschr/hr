...
//test
  ...
  */ 
 class JFormFieldMyuserandchildrenanddor extends JFormFieldList
 { 
     /** 
      * The form field type. 
      */ 
     public $type = 'Myuserandchildrenanddor'; 
...
$fieldlistusers = array('u.id', 'u.name', 'up.profile_value'); // add the field names to an array
...
$querymyuserandchildren->join('LEFT', $db->quoteName('#__user_profiles', 'up') . ' ON (' . $db->quoteName('up.user_id') . ' = ' . $db->quoteName('u.id') . ')');




 $querymyuserandchildren->where($db->quoteName('up.profile_key') . ' = ' . "'profile.dor'");  
...
$querymyuserandchildren->order($db->quoteName('u.name'));
   ...
   //$i=1;
		...
		foreach ($options as $i=>$option) {
                    $newdatevalue =   substr(JText::_($option->profile_value), 1, 10);
	            $options[$i]->value = JText::_($option->id) . 'the dor is'. $newdatevalue;
              //      $options[$i]->value = JText::_($option->id);
                        $options[$i]->text = JText::_($option->name);
                    //    $options[$i]->profilevalue = JText::_($option->profile_value);    
     
                   //     jexit();
                      //    $options[$i]->text = JText::_($option->profile_value);
		}
