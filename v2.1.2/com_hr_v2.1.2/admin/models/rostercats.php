...
/**
 * Methods supporting a list of rostercat records.
 *
 * @since  1.6
 */
class HrModelRostercats extends JModelList
{
	/**
	 * Constructor.
	 *
...
$stringofarrayofcatandsubcats = implode("' , '", $arrayofcatandsubcats);     
		...
		$query->from('#__hr_rostercat AS a');
...
$query->join('LEFT', $db->quoteName('#__hr_roster', 'r') . ' ON (' . $db->quoteName('a.roster_id') . ' = ' . $db->quoteName('r.id') . ')');
                ...
                $query->select('a.title AS rostercat_title');
 ...
 // Filter by position
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_rostercatitem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.rostercat')
				);
		}
*/
...
$query->where($db->quoteName('a.refcatid') . ' IN ' . 
        ...
        '(SELECT DISTINCT rc.refcatid as refcatid
                       FROM  #__hr_rostercat As rc 
                         where rc.refcatid = '. "'$mycatid'". ' and '.              
...
$query->where($db->quoteName('a.refcatid') . ' IN ' . 
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__hr_rostercat AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	/**
	 * Method to get a list of rostercats.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove rostercats the user shouldn't see
