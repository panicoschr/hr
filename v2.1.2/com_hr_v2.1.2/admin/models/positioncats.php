...
/**
 * Methods supporting a list of positioncat records.
 *
 * @since  1.6
 */
class HrModelPositioncats extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	//	$this->setState('filter.tag', $tag);
                ...
                $position_id = $this->getUserStateFromRequest($this->context . '.filter.position_id', 'filter_position_id', '');
                $this->setState('filter.position_id', $position_id);
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.language');
                ...
                $id .= ':' . $this->getState('filter.position_id');
...
$stringofarrayofcatandsubcats = implode("' , '", $arrayofcatandsubcats);     
		...
		$query->from('#__hr_positioncat AS a');
...
$query->join('LEFT', $db->quoteName('#__hr_position', 'p') . ' ON (' . $db->quoteName('a.position_id') . ' = ' . $db->quoteName('p.id') . ')');                
...
$query->select('(SELECT cats.title as catstitle
             ...
             where cats.id = a.refcatid)  AS refcatidtitle');  
                ...
                $query->select('a.title AS positioncat_title');
...
$position_id = $this->getState('filter.position_id');

        if (($position_id != NULL) || ($position_id != 0)){

            $query->where($db->quoteName('a.position_id') . ' =  ' . $position_id);
        }
 ...
 // Filter by position
      ...
      $itemselected = $this->getState('filter.listitem');
...
$query->where($db->quoteName('a.refcatid') . ' IN ' . 
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					'LEFT', $db->quoteName('#__hr_positioncatitem_tag_map', 'tagmap')
					. ' ON ' . $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hr.positioncat')
				);
		}
*/
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__hr_positioncat AS c ON c.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
	...
	/**
	 * Method to get a list of positioncats.
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove positioncats the user shouldn't see
