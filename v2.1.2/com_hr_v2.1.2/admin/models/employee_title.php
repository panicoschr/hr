...
/**
 * Item Model for an Employee_title.
 *
 * @since  1.6
 */
class HrModelEmployee_title extends JModelAdmin
{
	/**
	...
	/**
	 * The type alias for this content type (for example, 'com_hr.employee_title').
	 *
	 * @var      string
	 * @since    3.2
	 */
	public $typeAlias = 'com_hr.employee_title';
   ...
   if ($date1 > $date2) {
      ...
      public function checkOverlappingAbsences($data) {
        ...
        $query->select($db->quoteName(array('a.employee_id', 'u.name', 'a.datetime_from', 'a.datetime_to', 't.title', 'a.title_id')));
        $query->from($db->quoteName('#__hr_employee_title') . ' AS a');
...
//$query->where('(a.employee_id = '."'$employee_id'".' OR a.title_id = '."'$title_id'".')');
 ...
 $datetime_from  = new DateTime($datetime_from_string);
          $datetime_to  = new DateTime($datetime_to_string);  
            ...
            $count = $count + 1;
                    $keepDatesTrack = $keepDatesTrack . " " . $uname. " owns the title of " . $title_title. " from " . $formatedfromdate . " to " . $formatedtodate .  ", " ;
                }
            }
            if ($datetime_from == $datetime_to) {
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// Check if the employee_title was featured and update the #__hr_employee_title_frontpage table
			if ($featured == 1)
			{
				$db = $this->getDbo();
				$query = $db->getQuery(true)
					->insert($db->quoteName('#__hr_employee_title_frontpage'))
					->values($newId . ', 0');
				$db->setQuery($query);
				$db->execute();
			}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_hr.employee_title.' . (int) $record->id);
		}
	...
	protected function canEditState($record)
	{
		...
		// Check for existing employee_title.
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_hr.employee_title.' . (int) $record->id);
		}
		// New employee_title, so check against the category.
		elseif (!empty($record->catid))
		{
			return $user->authorise('core.edit.state', 'com_hr.category.' . (int) $record->catid);
		}
		// Default to component settings if neither employee_title nor category known.
		else
		{
	...
	protected function prepareTable($table)
	{
		...
		// Reorder the employee_titles within the category so the new employee_title is first
		if (empty($table->id))
		{
	 ...
	 */
	public function getTable($type = 'Employee_title', $prefix = 'HrTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$item->employee_titletext = trim($item->fulltext) != '' ? $item->introtext . "<hr id=\"system-readmore\" />" . $item->fulltext : $item->introtext;

                      
			if (!empty($item->id))
			{
				$item->tags = new JHelperTags;
				$item->tags->getTagIds($item->id, 'com_hr.employee_title');
			}
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_hr', '#__hr_employee_title', 'com_hr.item', $item->id);
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hr.employee_title', 'employee_title', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($this->getState('employee_title.id'))
		{
			$id = $this->getState('employee_title.id');

			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');

			// Existing record. Can only edit own employee_titles in selected categories.
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_hr.employee_title.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_hr')))
		{
			...
			// The controller has already verified this is an employee_title you can edit.
		...
		// Prevent messing with employee_title language and category when editing existing employee_title with associations
		$app = JFactory::getApplication();
		$assoc = JLanguageAssociations::isEnabled();

		// Check if employee_title is associated
		if ($this->getState('employee_title.id') && $app->isSite() && $assoc)
		{
			$associations = JLanguageAssociations::getAssociations('com_hr', '#__hr_employee_title', 'com_hr.item', $id);
	...
	protected function loadFormData()
	{
		...
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_hr.edit.employee_title.data', array());
       ...
       if (!isset($data->id)) {
			...
			// Pre-select some filters (Status, Category, Language, Access) in edit form if those have been selected in Employee_title Manager: Employee_titles
			if ($this->getState('employee_title.id') == 0)
			{
				$filters = (array) $app->getUserState('com_hr.employee_titles.filter');
				$data->set(
					'state',
					$app->input->getInt(
						'state',
   ...
   if ($data->id == NULL) {
            ...
            $query->where($db->quoteName('title') . ' = ' . "'Titles of Employees'");
            $db->setQuery($query);
            $results = $db->loadObjectList();
            foreach ($results as $result) {
		...
		$this->preprocessData('com_hr.employee_title', $data);
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (!isset($data['id']) || (int) $data['id'] == 0))
		{
			if ($data['alias'] == null)
			{
				...
				$table = JTable::getInstance('Employee_title', 'HrTable');
	...
	/**
	 * Method to toggle the featured setting of employee_titles.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		$table = $this->getTable('Employee_title', 'HrTable');

		try
		{
			$db = $this->getDbo();
			$query = $db->getQuery(true)
				->update($db->quoteName('#__hr_employee_title'))
			...
			if ((int) $value == 0)
			{
				...
				$query = $db->getQuery(true)
					->delete($db->quoteName('#__hr_employee_title_frontpage'))
			...
			{
				// First, we find out which of our new featured employee_titles are already featured.
				$query = $db->getQuery(true)
					->select('f.ms_id')
					->from('#__hr_employee_title_frontpage AS f')
					->where('ms_id IN (' . implode(',', $pks) . ')');
				$db->setQuery($query);

				$old_featured = $db->loadColumn();

				// We diff the arrays to get a list of the employee_titles that are newly featured
		...
		if ($assoc)
		{
			...
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) || $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_employee_title');
	...
	//	parent::cleanCache('mod_employee_titles_archive');
	//	parent::cleanCache('mod_employee_titles_categories');
	//	parent::cleanCache('mod_employee_titles_category');
	//	parent::cleanCache('mod_employee_titles_latest');
	//	parent::cleanCache('mod_employee_titles_news');
	//	parent::cleanCache('mod_employee_titles_popular');
	}
