...
/**
 * Item Model for an Positioncat.
 *
 * @since  1.6
 */
class HrModelPositioncat extends JModelAdmin
{
	/**
	...
	/**
	 * The type alias for this content type (for example, 'com_hr.positioncat').
	 *
	 * @var      string
	 * @since    3.2
	 */
	public $typeAlias = 'com_hr.positioncat';
   ...
   if ($date1 > $date2) {
      ...
      public function checkOverlappingAbsences($data) {
        ...
        $query->select($db->quoteName(array('a.position_id', 'p.title', 'a.datetime_from', 'a.datetime_to', 'c.title', 'a.refcatid')));
        $query->from($db->quoteName('#__hr_positioncat') . ' AS a');
 ...
 $datetime_from  = new DateTime($datetime_from_string);
          $datetime_to  = new DateTime($datetime_to_string);     
            ...
            $count = $count + 1;
                    $keepDatesTrack = $keepDatesTrack . " " . $position_title. " belongs to " . $category_title. " from " . $formatedfromdate . " to " . $formatedtodate .  ", " ;
                     }
            }
            if ($datetime_from == $datetime_to) {
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// Check if the positioncat was featured and update the #__hr_positioncat_frontpage table
			if ($featured == 1)
			{
				$db = $this->getDbo();
				$query = $db->getQuery(true)
					->insert($db->quoteName('#__hr_positioncat_frontpage'))
					->values($newId . ', 0');
				$db->setQuery($query);
				$db->execute();
			}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_hr.positioncat.' . (int) $record->id);
		}
	...
	protected function canEditState($record)
	{
		...
		// Check for existing positioncat.
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_hr.positioncat.' . (int) $record->id);
		}
		// New positioncat, so check against the category.
		elseif (!empty($record->catid))
		{
			return $user->authorise('core.edit.state', 'com_hr.category.' . (int) $record->catid);
		}
		// Default to component settings if neither positioncat nor category known.
		else
		{
	...
	protected function prepareTable($table)
	{
		...
		// Reorder the positioncats within the category so the new positioncat is first
		if (empty($table->id))
		{
	 ...
	 */
	public function getTable($type = 'Positioncat', $prefix = 'HrTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$item->positioncattext = trim($item->fulltext) != '' ? $item->introtext . "<hr id=\"system-readmore\" />" . $item->fulltext : $item->introtext;

                      
			if (!empty($item->id))
			{
				$item->tags = new JHelperTags;
				$item->tags->getTagIds($item->id, 'com_hr.positioncat');
			}
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_hr', '#__hr_positioncat', 'com_hr.item', $item->id);
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hr.positioncat', 'positioncat', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($this->getState('positioncat.id'))
		{
			$id = $this->getState('positioncat.id');

			// Existing record. Can only edit in selected categories.
			$form->setFieldAttribute('catid', 'action', 'core.edit');

			// Existing record. Can only edit own positioncats in selected categories.
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_hr.positioncat.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_hr')))
		{
			...
			// The controller has already verified this is an positioncat you can edit.
		...
		// Prevent messing with positioncat language and category when editing existing positioncat with associations
		$app = JFactory::getApplication();
		$assoc = JLanguageAssociations::isEnabled();

		// Check if positioncat is associated
		if ($this->getState('positioncat.id') && $app->isSite() && $assoc)
		{
			$associations = JLanguageAssociations::getAssociations('com_hr', '#__hr_positioncat', 'com_hr.item', $id);
	...
	protected function loadFormData()
	{
		...
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_hr.edit.positioncat.data', array());
   ...
   if ((isset($data->id)) && ($data->id != '0')) {
       ...
       if (!isset($data->id)) {
			...
			// Pre-select some filters (Status, Category, Language, Access) in edit form if those have been selected in Positioncat Manager: Positioncats
			if ($this->getState('positioncat.id') == 0)
			{
				$filters = (array) $app->getUserState('com_hr.positioncats.filter');
				$data->set(
					'state',
					$app->input->getInt(
						'state',
   ...
   if ($isset == false) {
            ...
            $query->where($db->quoteName('title') . ' = ' . "'Positions to Departments/Sections'");
            $db->setQuery($query);
            $results = $db->loadObjectList();
            foreach ($results as $result) {
		...
		$this->preprocessData('com_hr.positioncat', $data);
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (!isset($data['id']) || (int) $data['id'] == 0))
		{
			if ($data['alias'] == null)
			{
				...
				$table = JTable::getInstance('Positioncat', 'HrTable');
	...
	/**
	 * Method to toggle the featured setting of positioncats.
	 *
	...
	public function featured($pks, $value = 0)
	{
		...
		$table = $this->getTable('Positioncat', 'HrTable');

		try
		{
			$db = $this->getDbo();
			$query = $db->getQuery(true)
				->update($db->quoteName('#__hr_positioncat'))
			...
			if ((int) $value == 0)
			{
				...
				$query = $db->getQuery(true)
					->delete($db->quoteName('#__hr_positioncat_frontpage'))
			...
			{
				// First, we find out which of our new featured positioncats are already featured.
				$query = $db->getQuery(true)
					->select('f.ms_id')
					->from('#__hr_positioncat_frontpage AS f')
					->where('ms_id IN (' . implode(',', $pks) . ')');
				$db->setQuery($query);

				$old_featured = $db->loadColumn();

				// We diff the arrays to get a list of the positioncats that are newly featured
		...
		if ($assoc)
		{
			...
			foreach ($languages as $tag => $language)
			{
				if (empty($data->language) || $tag != $data->language)
				{
					...
					$field->addAttribute('type', 'modal_positioncat');
	...
	//	parent::cleanCache('mod_positioncats_archive');
	//	parent::cleanCache('mod_positioncats_categories');
	//	parent::cleanCache('mod_positioncats_category');
	//	parent::cleanCache('mod_positioncats_latest');
	//	parent::cleanCache('mod_positioncats_news');
	//	parent::cleanCache('mod_positioncats_popular');
	}
