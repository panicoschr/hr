...
if (!$editoroptions)
{
	...
	$params->show_machine_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the machine uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_machine_options']) && $this->item->attribs['show_machine_options'] != '')
{
	$params->show_machine_options = $this->item->attribs['show_machine_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "machine.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('machinetext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "machine.apply")
			{
				window.parent.jQuery("#machineEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<form action="<?php echo JRoute::_('index.php?option=com_hr&layout=' . $layout . $tmpl . '&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="item-form" class="form-validate">
 ...
 <?php echo JLayoutHelper::render('components.com_hr.helpers.edit.ip',  $this, JPATH_ADMINISTRATOR); ?>  
 
    
	<div class="form-horizontal">
		<?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_MACHINE_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('machinetext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_machine_options; ?>
