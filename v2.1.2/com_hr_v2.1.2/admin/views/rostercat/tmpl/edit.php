...
if (!$editoroptions)
{
	...
	$params->show_rostercat_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the rostercat uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_rostercat_options']) && $this->item->attribs['show_rostercat_options'] != '')
{
	$params->show_rostercat_options = $this->item->attribs['show_rostercat_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "rostercat.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('rostercattext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "rostercat.apply")
			{
				window.parent.jQuery("#rostercatEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<?php echo JLayoutHelper::render('components.com_hr.helpers.edit.roster_category',  $this, JPATH_ADMINISTRATOR); ?>      
	...
	<div class="form-horizontal">
		...
		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_ROSTERCAT_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('rostercattext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_rostercat_options; ?>
