...
if (!$editoroptions)
{
	...
	$params->show_positioncat_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the positioncat uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_positioncat_options']) && $this->item->attribs['show_positioncat_options'] != '')
{
	$params->show_positioncat_options = $this->item->attribs['show_positioncat_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "positioncat.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('positioncattext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "positioncat.apply")
			{
				window.parent.jQuery("#positioncatEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<?php echo JLayoutHelper::render('components.com_hr.helpers.edit.position_category',  $this, JPATH_ADMINISTRATOR); ?>      
  
    
	<div class="form-horizontal">
		<?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_POSITIONCAT_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('positioncattext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_positioncat_options; ?>
