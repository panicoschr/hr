...
if (!$editoroptions)
{
	...
	$params->show_employee_title_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the employee_title uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_employee_title_options']) && $this->item->attribs['show_employee_title_options'] != '')
{
	$params->show_employee_title_options = $this->item->attribs['show_employee_title_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "employee_title.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('employee_titletext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "employee_title.apply")
			{
				window.parent.jQuery("#employee_titleEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<?php echo JLayoutHelper::render('components.com_hr.helpers.edit.employee_title',  $this, JPATH_ADMINISTRATOR); ?>      
	...
	<div class="form-horizontal">
		...
		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_EMPLOYEE_TITLE_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('employee_titletext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_employee_title_options; ?>
