...
if (!$editoroptions)
{
	...
	$params->show_employee_absence_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the employee_absence uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_employee_absence_options']) && $this->item->attribs['show_employee_absence_options'] != '')
{
	$params->show_employee_absence_options = $this->item->attribs['show_employee_absence_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "employee_absence.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('employee_absencetext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "employee_absence.apply")
			{
				window.parent.jQuery("#employee_absenceEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<?php echo JLayoutHelper::render('components.com_hr.helpers.edit.employee_absence',  $this, JPATH_ADMINISTRATOR); ?>      
 
    
	<div class="form-horizontal">
		<?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_EMPLOYEE_ABSENCE_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('employee_absencetext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_employee_absence_options; ?>
		...
		<?php if ($this->canDo->get('core.admin')) : ?>
			<?php 
                        ...
                        echo JHtml::_('bootstrap.addTab', 'myTab', 'permissions', JText::_('COM_HR_FIELDSET_RULES')); ?>
