...
$columns   = 10;
$view_name = 'employee_rosters';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_hr&task=employee_rosters.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'employee_rosterList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
		...
		<?php else : ?>
			<table class="table table-striped" id="employee_rosterList">
					...
					<tr>
						...
						<th>
							<?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_EMPLOYEE_LABEL', 'username', $listDirn, $listOrder); ?>
                                                ...
                                                <th>
						<?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_DATETIME_FROM_LABEL', 'a.datetime_from', $listDirn, $listOrder); ?>
				         	</th>
	                                         <th>
						<?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_DATETIME_TO_LABEL', 'a.datetime_to', $listDirn, $listOrder); ?>
					         </th>
			                        <th>
                                                        <?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_WORK_STATUS_LABEL', 'a.work_status', $listDirn, $listOrder); ?>
                                                </th>                                                    
                                                <th>
                                                        <?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_ABSTYPE_LABEL', 'ab.abstype', $listDirn, $listOrder); ?>
                                                </th>
                                             <th>
                                                        <?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_FINAL_APPROVAL_STATUS_LABEL', 'ea.final_approval_status', $listDirn, $listOrder); ?>
                                                </th>                                                
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canEdit    = $user->authorise('core.edit',       'com_hr.employee_roster.' . $item->id);
					$canCheckin = $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
					$canEditOwn = $user->authorise('core.edit.own',   'com_hr.employee_roster.' . $item->id) && $item->created_by == $userId;
					$canChange  = $user->authorise('core.edit.state', 'com_hr.employee_roster.' . $item->id) && $canCheckin;
					?>
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
						...
						<td class="center">
							<div class="btn-group">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'employee_rosters.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
								<?php echo JHtml::_('hradministrator.featured', $item->featured, $i, $canChange, $view_name); ?>
								<?php // Create dropdown items and render the dropdown list.
								if ($canChange)
								{
									JHtml::_('actionsdropdown.' . ((int) $item->state === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'employee_rosters');
									JHtml::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'employee_rosters');
						...
						<td class="has-context">
							<div class="pull-left break-word">
								<?php if ($item->checked_out) : ?>
									<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'employee_rosters.', $canCheckin); ?>
								<?php endif; ?>
								...
								<?php if ($canEdit || $canEditOwn) : ?>
									<a class="hasTooltip" href="<?php echo JRoute::_('index.php?option=com_hr&task=employee_roster.edit&id=' . $item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
