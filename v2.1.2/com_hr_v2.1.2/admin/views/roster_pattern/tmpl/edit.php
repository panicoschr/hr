...
if (!$editoroptions)
{
	...
	$params->show_roster_pattern_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the roster_pattern uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_roster_pattern_options']) && $this->item->attribs['show_roster_pattern_options'] != '')
{
	$params->show_roster_pattern_options = $this->item->attribs['show_roster_pattern_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "roster_pattern.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('roster_patterntext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "roster_pattern.apply")
			{
				window.parent.jQuery("#roster_patternEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<script type="text/javascript">
  ...
  var strtimefrom = document.getElementById("jform_idtimefrom_lbl").value;
document.getElementById("jform_idtimefrom_text_lbl").value = strtimefrom.substring(11, 16);
   
   
  var strtimefrom = document.getElementById("jform_idtimeto_lbl").value;
document.getElementById("jform_idtimeto_text_lbl").value = strtimefrom.substring(11, 16);   
...
<?php echo JLayoutHelper::render('components.com_hr.helpers.edit.roster_pattern',  $this, JPATH_ADMINISTRATOR); ?>      
 
    
	<div class="form-horizontal">
		<?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_ROSTER_PATTERN_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('roster_patterntext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_roster_pattern_options; ?>
