...
$columns   = 10;
$view_name = 'employee_attendances';

$groupselected = $this->escape($this->state->get('filter.groupselected'));
...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_hr&task=employee_attendances.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'employee_attendanceList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
			...
			<table class="table table-striped" id="employee_attendanceList">
				<thead>
					<tr>
						...
						<th width="40%" class="nowrap has-context">
							<?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_LOG_IN_LABEL', 'a.log_in', $listDirn, $listOrder); ?>
						</th>                                                
                                                
						<th width="40%" class="nowrap has-context">
							<?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_LOG_OUT_LABEL', 'a.log_out', $listDirn, $listOrder); ?>
						</th> 
                        <th>
<?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_DURATION_LABEL', 'minduration', $listDirn, $listOrder); ?>
                        </th>	                                                
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canEdit    = $user->authorise('core.edit',       'com_hr.employee_attendance.' . $item->id);
					$canCheckin = $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
					$canEditOwn = $user->authorise('core.edit.own',   'com_hr.employee_attendance.' . $item->id) && $item->created_by == $userId;
					$canChange  = $user->authorise('core.edit.state', 'com_hr.employee_attendance.' . $item->id) && $canCheckin;
					?>
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
						...
						<td class="center">
							<div class="btn-group">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'employee_attendances.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
								<?php echo JHtml::_('hradministrator.featured', $item->featured, $i, $canChange, $view_name); ?>
								<?php // Create dropdown items and render the dropdown list.
								if ($canChange)
								{
									JHtml::_('actionsdropdown.' . ((int) $item->state === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'employee_attendances');
									JHtml::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'employee_attendances');
						...
						<td class="has-context">
							<div class="pull-left break-word">
								<?php if ($item->checked_out) : ?>
									<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'employee_attendances.', $canCheckin); ?>
								<?php endif; ?>
								...
								<?php if ($canEdit || $canEditOwn) : ?>
									<a class="hasTooltip" href="<?php echo JRoute::_('index.php?option=com_hr&task=employee_attendance.edit&id=' . $item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
                  ...
                  <td class="has-context">
							<div class="pull-left break-word">   
                                    ...
                                    echo $hours . ':' . $minutes;
                                    //      echo $item->minduration/60;    
                                    //       echo (($item->minduration%60)*60);    


                                    /*
                                      if (strlen($item->hourduration) == '1')
                                      {
                                      echo '0';
                                      }
                                      echo $item->hourduration.':';
                                      if (strlen($item->minduration) == '1')
                                      {
                                      echo '0';
                                      }
                                      echo $item->minduration;
                                     */
