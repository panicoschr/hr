...
if (!$editoroptions)
{
	...
	$params->show_employee_roster_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the employee_roster uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_employee_roster_options']) && $this->item->attribs['show_employee_roster_options'] != '')
{
	$params->show_employee_roster_options = $this->item->attribs['show_employee_roster_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "employee_roster.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('employee_rostertext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "employee_roster.apply")
			{
				window.parent.jQuery("#employee_rosterEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<form action="<?php echo JRoute::_('index.php?option=com_hr&layout=' . $layout . $tmpl . '&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="item-form" class="form-validate">
 ...
 <?php echo JLayoutHelper::render('components.com_hr.helpers.edit.employee_roster',  $this, JPATH_ADMINISTRATOR); ?>      
	...
	<div class="form-horizontal">
		...
		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_EMPLOYEE_ROSTER_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('employee_rostertext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_employee_roster_options; ?>
