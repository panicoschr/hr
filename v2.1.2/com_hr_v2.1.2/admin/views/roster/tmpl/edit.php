...
if (!$editoroptions)
{
	...
	$params->show_roster_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the roster uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_roster_options']) && $this->item->attribs['show_roster_options'] != '')
{
	$params->show_roster_options = $this->item->attribs['show_roster_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "roster.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('rostertext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "roster.apply")
			{
				window.parent.jQuery("#rosterEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<form action="<?php echo JRoute::_('index.php?option=com_hr&layout=' . $layout . $tmpl . '&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="item-form" class="form-validate">
	...
	<div class="form-horizontal">
		...
		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_ROSTER_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('rostertext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_roster_options; ?>
