...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_hr&task=plancats.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'plancatList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}

$assoc = JLanguageAssociations::isEnabled(); $assoc = false;  $lang  = false;  //added by panikos to remove association and language 
?>

<form action="<?php echo JRoute::_('index.php?option=com_hr&view=plancats'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php endif;?>
		...
		<?php else : ?>
			<table class="table table-striped" id="plancatList">
				<thead>
					<tr>
						...
						<th width="1%" style="min-width:55px" class="nowrap center">
							<?php echo JHtml::_('searchtools.sort', 'JSTATUS', 'a.state', $listDirn, $listOrder); ?>
                                         ...
                                         <th>
						<?php echo JHtml::_('searchtools.sort', 'COM_HR_ENTITLEMENT_PLAN_ID_LABEL', 'b.title', $listDirn, $listOrder); ?>
					</th>                                                
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canEdit    = $user->authorise('core.edit',       'com_hr.plancat.' . $item->id);
					$canCheckin = $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
					$canEditOwn = $user->authorise('core.edit.own',   'com_hr.plancat.' . $item->id) && $item->created_by == $userId;
					$canChange  = $user->authorise('core.edit.state', 'com_hr.plancat.' . $item->id) && $canCheckin;
					?>
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
						...
						<td class="center">
							<div class="btn-group">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'plancats.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
								<?php echo JHtml::_('hradministrator.featured', $item->featured, $i, $canChange, $view_name); ?>
								<?php // Create dropdown items and render the dropdown list.
								if ($canChange)
								{
									JHtml::_('actionsdropdown.' . ((int) $item->state === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'plancats');
									JHtml::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'plancats');
						...
						<td class="has-context">
							<div class="pull-left break-word">
								<?php if ($item->checked_out) : ?>
									<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'plancats.', $canCheckin); ?>
								<?php endif; ?>
								...
								<?php if ($canEdit || $canEditOwn) : ?>
									<a class="hasTooltip" href="<?php echo JRoute::_('index.php?option=com_hr&task=plancat.edit&id=' . $item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
