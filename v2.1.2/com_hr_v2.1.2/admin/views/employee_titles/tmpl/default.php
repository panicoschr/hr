...
$columns   = 10;
$view_name = 'employee_titles';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_hr&task=employee_titles.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'employee_titleList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}

$assoc = JLanguageAssociations::isEnabled(); $assoc = false;  $lang  = false;  //added by panikos to remove association and language 
?>

<form action="<?php echo JRoute::_('index.php?option=com_hr&view=employee_titles'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php endif;?>
		...
		<?php else : ?>
			<table class="table table-striped" id="employee_titleList">
				<thead>
					<tr>
						...
						<th>
							<?php echo JHtml::_('searchtools.sort', 'COM_HR_FIELD_TITLE_LABEL', 't.title', $listDirn, $listOrder); ?>
						</th>                                                
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canEdit    = $user->authorise('core.edit',       'com_hr.employee_title.' . $item->id);
					$canCheckin = $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
					$canEditOwn = $user->authorise('core.edit.own',   'com_hr.employee_title.' . $item->id) && $item->created_by == $userId;
					$canChange  = $user->authorise('core.edit.state', 'com_hr.employee_title.' . $item->id) && $canCheckin;
					?>
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
						...
						<td class="center">
							<div class="btn-group">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'employee_titles.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
								<?php echo JHtml::_('hradministrator.featured', $item->featured, $i, $canChange, $view_name); ?>
								<?php // Create dropdown items and render the dropdown list.
								if ($canChange)
								{
									JHtml::_('actionsdropdown.' . ((int) $item->state === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'employee_titles');
									JHtml::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'employee_titles');
						...
						<td class="has-context">
							<div class="pull-left break-word">
								<?php if ($item->checked_out) : ?>
									<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'employee_titles.', $canCheckin); ?>
								<?php endif; ?>
								...
								<?php if ($canEdit || $canEditOwn) : ?>
									<a class="hasTooltip" href="<?php echo JRoute::_('index.php?option=com_hr&task=employee_title.edit&id=' . $item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
								...
								<div class="small">
									<?php echo JText::_('JCATEGORY') . ": " . $this->escape($item->category_title); ?>
                                                ...
                                                <td class="has-context">
                                                       ...
                                                       <?php echo $this->escape($item->ttitle); ?>                                                                
