...
if (!$editoroptions)
{
	...
	$params->show_plan_entitlement_options = '1';
	$params->show_urls_images_backend = '0';
	$params->show_urls_images_frontend = '0';
}

// Check if the plan_entitlement uses configuration settings besides global. If so, use them.
if (isset($this->item->attribs['show_publishing_options']) && $this->item->attribs['show_publishing_options'] != '')
{
	$params->show_publishing_options = $this->item->attribs['show_publishing_options'];
}

if (isset($this->item->attribs['show_plan_entitlement_options']) && $this->item->attribs['show_plan_entitlement_options'] != '')
{
	$params->show_plan_entitlement_options = $this->item->attribs['show_plan_entitlement_options'];
}
...
JFactory::getDocument()->addScriptDeclaration('
	...
	{
		if (task == "plan_entitlement.cancel" || document.formvalidator.isValid(document.getElementById("item-form")))
		{
			jQuery("#permissions-sliders select").attr("disabled", "disabled");
			' . $this->form->getField('plan_entitlementtext')->save() . '
	                          
			Joomla.submitform(task, document.getElementById("item-form"));

			if (task !== "plan_entitlement.apply")
			{
				window.parent.jQuery("#plan_entitlementEdit' . (int) $this->item->id . 'Modal").modal("hide");
			}
		}
	};
');
...
<form action="<?php echo JRoute::_('index.php?option=com_hr&layout=' . $layout . $tmpl . '&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="item-form" class="form-validate">
 ...
 <?php echo JLayoutHelper::render('components.com_hr.helpers.edit.plan_entitlement',  $this, JPATH_ADMINISTRATOR); ?>      
	<div class="form-horizontal">
		<?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HR_PLAN_ENTITLEMENT_CONTENT')); ?>
		<div class="row-fluid">
			<div class="span9">
				<fieldset class="adminform">
					<?php echo $this->form->getInput('plan_entitlementtext'); ?>
				</fieldset>
			</div>
			<div class="span3">
		...
		<?php $this->show_options = $params->show_plan_entitlement_options; ?>
...
document.getElementById("jform_idmeasureunitandid").value = document.getElementById("jform_idmeasureunit").value.concat('the id is').concat(document.getElementById("jform_id_absence_id").value);
        ...
        var abstype1 = document.getElementById("jform_idmeasureunitandid").value;

       
        if ((abstype1.indexOf("Days") > -1))
        {
            planentlenablefields();
            actionOnFromToFields();
        }
       else if ((abstype1.indexOf("Hours_minutes") > -1))
    {       
         planentlenablefields;
         actionOnFromToFields();
        } else
