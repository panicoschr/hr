...
class HrControllerEmployee_entitlement extends JControllerForm
{
	/**
	 * Class constructor.
	 *
	...
	public function __construct($config = array())
	{
		...
		// An employee_entitlement edit form can come from the employee_entitlements or featured view.
		// Adjust the redirect view on the value of 'return' in the request.
		if ($this->input->get('return') == 'featured')
		{
			$this->view_list = 'featured';
			$this->view_item = 'employee_entitlement&return=featured';
		}
	}
  ...
  $app = JFactory::getApplication();
        ...
        $model = $this->getModel('employee_entitlement');
        ...
        $msg_all_fields_have_a_value = 'Please make sure all '
                . 'required fields have a value';
      
        $msg_different_year_error= 'From (Date) and To (Date) must '
        ...
        $url = 'index.php?option=com_hr&view=employee_entitlements&layout=edit';
...
if (($allFiledsHaveAValue) && ($dates_ok) && ($all_date_fields_have_same_year)){
           ...
           $this->setRedirect($url);
            ...
            if ((!$allFiledsHaveAValue) || (!$dates_ok) || (!$all_date_fields_have_same_year)) {
                ...
                if (!$allFiledsHaveAValue) {
                    $this->setRedirect($url, $msg_all_fields_have_a_value);
                    return false;
                }                
            }
        }
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit', 'com_hr.employee_entitlement.' . $recordId))
		{
			return true;
		}

		// Check edit own on the record asset (explicit or inherited)
		if ($user->authorise('core.edit.own', 'com_hr.employee_entitlement.' . $recordId))
		{
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Employee_entitlement', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_hr&view=employee_entitlements' . $this->getRedirectToListAppend(), false));
