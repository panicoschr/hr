...
class HrControllerEmployee_absence extends JControllerForm
{
	/**
	 * Class constructor.
	 *
	...
	public function __construct($config = array())
	{
		...
		// An employee_absence edit form can come from the employee_absences or featured view.
		// Adjust the redirect view on the value of 'return' in the request.
		if ($this->input->get('return') == 'featured')
		{
			$this->view_list = 'featured';
			$this->view_item = 'employee_absence&return=featured';
		}
	}
    ...
    require_once JPATH_COMPONENT.'/helpers/hr.php';                   
        ...
        $model = $this->getModel('employee_absence');
  ...
  //  ////var_depr_dump($data['datetime_to']);
   //    jexit();
        ...
        $url = 'index.php?option=com_hr&view=employee_absence&layout=edit';

        if (($dates_ok) && ($overlapping == 0)) {
            parent::save($data, $urlVar);
            if ($data['final_approval_status'] == 'Approved'){
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit', 'com_hr.employee_absence.' . $recordId))
		{
			return true;
		}

		// Check edit own on the record asset (explicit or inherited)
		if ($user->authorise('core.edit.own', 'com_hr.employee_absence.' . $recordId))
		{
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Employee_absence', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_hr&view=employee_absences' . $this->getRedirectToListAppend(), false));
